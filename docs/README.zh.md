...
[![tests](https://img.shields.io/badge/tests-1510%20passing-2ea44f)](#已验证)
...
真正覆盖了这次改动的 jest 运行也算数。随后，在完全相同的这棵树上已经观察到的失败会被重新
援引而不是重新运行：同一次拒绝只花一秒，而不是再跑一遍整个套件。
...
```
make check    # lint · docs gate · slop gate · polyglot gate · knowledge · 1510 个测试 · 33 项 doctor 检查 · budget
```
