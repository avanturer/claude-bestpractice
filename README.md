...
[![version](https://img.shields.io/badge/version-1.69.3-black)](https://github.com/avanturer/claude-bestpractice/releases)
[![tests](https://img.shields.io/badge/tests-2161%20passing-2ea44f)](#verified)
...
**A repository with two projects has two suites.** `test_commands` in
...
| `claude-bp database` | Create this worktree's own database, if the server has not got it (needs `psql`; otherwise `worktree_setup`) |
...
```
make check    # lint · docs gate · slop gate · polyglot gate · knowledge · 2161 tests · 36 doctor checks · budget
```
